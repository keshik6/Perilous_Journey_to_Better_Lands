...
# Data preprocessing
We will process the data obtained from https://missingmigrants.iom.int/ into a useable format. We quickly did an initial visualization using QGIS to get a sense of the data and noticed some incorrect data points and will be filtered out in this step. 
...
map
```
The plot reveals clusters in reported events of migrant missing and death cases around the Mediterranean, North African and US-Mexico regions. It is very strange to note that there are no migrant death/missing events reported around the South Eastern Indian Ocean closer to Austrailia and Papua New Guinea. We will address these issues in the conclusion section.
...
# We used `factor(match(`Reported Month`, month.abb))` to order the months, but there might be a more elegant solution. We piped the data instead of doing x=factor(match()).
med_events %>% 
  ggplot(aes(x=factor(match(`Reported Month`, month.abb)))) + geom_bar() + facet_wrap(~`Migration Route`) + xlab("Reported Month")
...
You might have wondered why we are not focusing on time-series analysis (Based on years). The primary reason is that with time the tecchnology, awareness and methods used to report migrant death and missing events has increased. i.e.: Though we have more migrant death and missing events reported in 2015 compared to 2014, it does not mean that 2014 actually had less migrant death and missing events, but it is primarly because we have better methods and technology to track these events in 2015 compared to 2014. So time - series analysis will not yield correct insights. 
...
# Faceted by month, plot of total dead and missing
# When exporting as png, use scale_size_continuous instead of scale_size_area() if the points are too small.
# Faceted months not ordered properly.
map + geom_point(alpha = 0.4, aes(x=Longitude, y=Latitude, size=med_events$`Total Dead and Missing`, colour="red"), data=med_events) + scale_size_area() + facet_wrap(~`Reported Month`)

map + geom_point(alpha = 0.4, aes(x=Longitude, y=Latitude, size=med_events$`Total Dead and Missing`, colour="red"), data=med_events) + scale_size_area() + facet_wrap(~`Reported Year`)
```


# Try to find any trends in migrant flow and fatalities
The data of missing migrants that we are using does not encode any information on the nationality of the victims. So we are not able to exclusively formulate any methodology to understand trends between country of origin and missing migrants. So we are making an assumption that, 
Proportion of Fatalities where victims belong to country Z = Proportion of country Z asylum seekers in the EU. To give a concrete example, if 20% of asylum seekers in the EU are from Iraq, then 20% of people who die in the mediterrenean are Iraqis. 
...
asylum_seekers %>% 
  ...
  xlab("Destination") + ylab("Number of Asylum Seeker") +
...
We will understand the migrant flow by incorporating the underlying geographical layer in this section. First we extract our filtered data and we create a graph from it. Do note that Edge.csv and Nodes.csv were manually created by us. In this part we will walkthrough on how we created our Sankey diarams and we will show a concrete example as well(Germany)
```{r, message=FALSE, warning=FALSE}
...
subGraph %>%
  ...
  xlim(c(-20,50)) + ylim(c(0,70)) + scale_edge_width_identity() + theme_void()
...
We also make use of ggsave to save our plots. You can find all the plots at https://github.com/keshik6/Perilous_Journey_to_Better_Lands .These sankey diagrams are shown in the poster as well
```{r warning=FALSE, message=FALSE}
...
If you observe these Sankey diagrams for migrant flow through the mediterrenean sea its quite interesting to note that the thickness/ visual emphasis on the flow of the arrows that end up in the mediterrenean sea is very minute. That is the number of fatalities per country normalized based on the total number of people of the particular country who successfully seek asylum in the EU is insignificant. Our analysis becomes very interesting at this point. Though the above proportions appear very small numerically we have more than 22500 migrant death and missing toll from 2014 - 2018 in the Mediterrenean region. Do the authorities and governments understand the seriousness of this situation or does the percentages behind being small allow them to consider rescue operations as trivial relative to the investment of money and labour that these operations incur.


# Mediterrenean Search and Rescue(SAR) Operations
```{r, message=FALSE, warning=FALSE}
...
It is quite useful to visualize the number of rescued people by different Naval Units in the Meditteranean Sea from 2014-2018.
```{r, message=FALSE, warning=FALSE}
...
There is a large drop in the number of rescued people in the mediterenean from 2016 to 2017. We do not have sufficient evidance to explain the reasoning behind this.
